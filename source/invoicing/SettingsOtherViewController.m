...
@implementation SettingsOtherViewController
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = @"Other Settings";
    ...
    // Setup scroll view
    [self.scrollView setContentSize:CGSizeMake(self.scrollView.frame.size.width, 540)];
...
- (void)openSettings
{
    ...
    if (![self.fetchedResultsController performFetch:&error]) {
        ...
        for (NSMutableDictionary *settings in self.fetchedResultsController.fetchedObjects) {
            ...
            if ([[settings valueForKey:@"name"] isEqual:@"other:bcc"]) {
                
                UIOtherBCC.text = [settings valueForKey:@"value"];
            }
            
            if ([[settings valueForKey:@"name"] isEqual:@"other:index"]) {
                
                UIOtherIndex.text = [settings valueForKey:@"value"];
            }
            
            if ([[settings valueForKey:@"name"] isEqual:@"other:bankdetails"]) {
                
                UIOtherIncludeBankDetails.on = [[settings valueForKey:@"value"] isEqual:@"on"] ? TRUE : FALSE;
            }
            
            if ([[settings valueForKey:@"name"] isEqual:@"other:productcode"]) {
                
                UIOtherIncludeProductCode.on = [[settings valueForKey:@"value"] isEqual:@"on"] ? TRUE : FALSE;
            }
            
            if ([[settings valueForKey:@"name"] isEqual:@"other:currency"]) {
                
                UIOtherCurrency.text = [settings valueForKey:@"value"];
            }
...
- (void)saveSettings
{
    ...
    [new_settings addObject:[[[NSMutableDictionary alloc] initWithObjectsAndKeys:@"other:bcc", @"name", UIOtherBCC.text, @"value",nil] autorelease]];
    [new_settings addObject:[[[NSMutableDictionary alloc] initWithObjectsAndKeys:@"other:index", @"name", [NSString stringWithFormat:@"%i", [UIOtherIndex.text intValue]], @"value",nil] autorelease]];
    [new_settings addObject:[[[NSMutableDictionary alloc] initWithObjectsAndKeys:@"other:bankdetails", @"name", UIOtherIncludeBankDetails.on ? @"on" : @"off", @"value",nil] autorelease]];
    [new_settings addObject:[[[NSMutableDictionary alloc] initWithObjectsAndKeys:@"other:productcode", @"name", UIOtherIncludeProductCode.on ? @"on" : @"off", @"value",nil] autorelease]];
    [new_settings addObject:[[[NSMutableDictionary alloc] initWithObjectsAndKeys:@"other:currency", @"name", UIOtherCurrency.text, @"value",nil] autorelease]];
